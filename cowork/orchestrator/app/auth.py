...
def _auth_headers(authorization: str, request_id: str | None = None) -> dict[str, str]:
...
async def _verify_with_core_api(authorization: str, request_id: str | None = None) -> None:
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            response = await client.get(
                f"{base_url}/auth/me",
                headers=_auth_headers(authorization, request_id=request_id),
            )
...
async def require_auth(
    ...
    await _verify_with_core_api(normalized, request_id=request_id)
    return normalized
